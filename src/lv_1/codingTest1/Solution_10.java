	...
	*/
    ...
    public int solution(String[][] board, int h, int w) {
        ...
        for (int i = 0; i < 4; i++) {
        	ddh = h + dh[i];
        	ddw = w + dw[i];
        	if (ddh >= 0 && ddw >= 0 && ddh < length && ddw < length) {
            	if (board[h][w].equals(board[ddh][ddw])) {
            		answer++;
            	}
        	}
        }
     ...
     *  
     *  나는 약간 바보가 맞을지도...
     *  계속 안 돼서 대체 내가 뭘 잘못했을까 계속 생각했는데 아무리 봐도 답이 안 나오는 거다.
     *  알고 보니 for (int i = 0; i < 4; i++) 문에 i 영역을 3 으로 걸어두고 계속 고민하고 있었다.
     *  이런 사소함은 대체 해결법이 뭘까....
     *  내 식에 자신감이 없나?
     *  내가 뭔가 빠트리고 있을 거라고 생각해서 사소한 것들을 보지 못하는 걸까.
     *  
     */
}
